...
public class AnalogClockView: UIView {
    ...
    override public func draw(_ rect: CGRect) {
        ...
        instanceLayerFat.frame = CGRect(x: midXFat, y: 3.0, width: layerWidthFat, height: layerWidthFat)
        instanceLayerFat.backgroundColor = UIColor.systemYellow.cgColor
        instanceLayerFat.cornerRadius = layerWidthFat / 2
...
//        replicatorLayerThin.addSublayer(instanceLayerThin)
        ...
        minuteHandLayer.cornerRadius = 2.5
        // Puts the center of the rectangle in the center of the clock
        ...
        if rect.size.width > rect.size.height {
            hourHandLayer.bounds = CGRect(x: 0, y: 0, width: 6, height: (rect.size.height / 2) - (rect.size.height * 0.2))
            minuteHandLayer.bounds = CGRect(x: 0, y: 0, width: 5, height: (rect.size.height / 2) - (rect.size.height * 0.1))
            secondHandLayer.bounds = CGRect(x: 0, y: 0, width: 3, height: rect.size.height / 2)
        } else {
            hourHandLayer.bounds = CGRect(x: 0, y: 0, width: 6, height: (rect.size.width / 2) - (rect.size.width * 0.2))
            minuteHandLayer.bounds = CGRect(x: 0, y: 0, width: 5, height: (rect.size.width / 2) - (rect.size.width * 0.1))
